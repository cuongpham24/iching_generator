# -*- coding: utf-8 -*-
# rewrite from https://github.com/wolfhong/LunarCalendar/blob/master/lunarcalendar/

import datetime
from ._compact import unicode_compatible
from datetime import datetime
from ._data import heavenlyStems, earthlyBranches, numHeaven, numEarth
...
class Converter(object):
    ...
    def Solar2LunarString(solar):
        ...
        day = Sexagenary.translateLunarDate(lunarYear, lunarMonth, lunarDate, isleap=isLeap)
        month = Sexagenary.translateLunarMonth(lunarYear, lunarMonth)
        year = Sexagenary.translateLunarYear(lunarYear)
...
class Sexagenary(object):
    ...
    def getString(heavenlyStem, earthlyBranch):
        return Sexagenary.heavenlyStems[heavenlyStem] + " " + Sexagenary.earthlyBranches[earthlyBranch]

    @staticmethod
    def translateLunarYear(lunarYear):
        diff = Sexagenary.refYear - lunarYear
        # Count forward if year is the future of referenceYear
        if diff <= 0:
            # Return heavenlyStem, earthlyBranch
            return abs(diff) % Sexagenary.numHeaven, abs(diff) % Sexagenary.numEarth
        # Count backward if year is the past of referenceYear
        else:
            # Return heavenlyStem, earthlyBranch
            return (Sexagenary.numHeaven - diff % Sexagenary.numHeaven) % Sexagenary.numHeaven, \
                    (Sexagenary.numEarth - diff % Sexagenary.numEarth) % Sexagenary.numEarth
    
    @staticmethod
    def translateLunarMonth(lunarYear, lunarMonth):
        yearHeavenlyStem, _ = Sexagenary.translateLunarYear(lunarYear)
    ...
    def translateLunarDate(lunarYear, lunarMonth, lunarDate, isleap=False):
        ...
        numDays = datetime(Sexagenary.refSolar.year, 
                           Sexagenary.refSolar.month, 
                           Sexagenary.refSolar.day) - datetime(solarDate.year, solarDate.month, solarDate.day)
        diff = numDays.days
        # Count forward if year is the future of referenceYear
        if diff <= 0:
            # Return heavenlyStem, earthlyBranch
