# -*- coding: utf-8 -*-
# part of this code is rewriten from https://github.com/wolfhong/LunarCalendar/blob/master/lunarcalendar/

import datetime
from ._compact import unicode_compatible
from datetime import datetime
from ._constant import heavenlyStems, earthlyBranches, numHeaven, numEarth
...
class Converter(object):
    ...
    def Solar2Sexagenary(solar):
        ...
        # Convert to sexagenary calendar
        day = SexagenaryBasic.translateLunarDate(lunarYear, lunarMonth, lunarDate, isleap=isLeap)
        month = SexagenaryBasic.translateLunarMonth(lunarYear, lunarMonth)
...
class SexagenaryBasic(object):
    ...
    refYear = 1924      # Giap Ty
    refMonth = 1        # Dinh Mao
    refDate = 11        # Giap Ty
    refSolar = Converter.Lunar2Solar(Lunar(refYear, refMonth, refDate))
    
    @staticmethod
    def getString(heavenlyStem, earthlyBranch):
        return SexagenaryBasic.heavenlyStems[heavenlyStem] + " " + SexagenaryBasic.earthlyBranches[earthlyBranch]

    @staticmethod
    def translateLunarYear(lunarYear):
        diff = SexagenaryBasic.refYear - lunarYear
        # Count forward if year is the future of referenceYear
        if diff <= 0:
            # Return heavenlyStem, earthlyBranch
            return abs(diff) % SexagenaryBasic.numHeaven, abs(diff) % SexagenaryBasic.numEarth
        # Count backward if year is the past of referenceYear
        else:
            # Return heavenlyStem, earthlyBranch
            return (SexagenaryBasic.numHeaven - diff % SexagenaryBasic.numHeaven) % SexagenaryBasic.numHeaven, \
                    (SexagenaryBasic.numEarth - diff % SexagenaryBasic.numEarth) % SexagenaryBasic.numEarth
    
    @staticmethod
    def translateLunarMonth(lunarYear, lunarMonth):
        yearHeavenlyStem, _ = SexagenaryBasic.translateLunarYear(lunarYear)
        # Calculate starting HeavenlyStem
        start = (yearHeavenlyStem % 5) * 2 + 2
        # Use the calculated start plus displacement in month to get monthHeavenlyStem
        monthHeavenlyStem = (start + lunarMonth - 1) % 10
        # Calculate monthEarthlyBranch
        monthEarthlyBranch = (lunarMonth + 1) % 12
        # Return heavenlyStem, earthlyBranch
        return monthHeavenlyStem, monthEarthlyBranch

    @staticmethod
    def translateLunarDate(lunarYear, lunarMonth, lunarDate, isleap=False):
        # Get the number of julian day using solar calendar
        solarDate = Converter.Lunar2Solar(Lunar(lunarYear, lunarMonth, lunarDate, isleap=isleap))
        numDays = datetime(SexagenaryBasic.refSolar.year, 
                           SexagenaryBasic.refSolar.month, 
                           SexagenaryBasic.refSolar.day) - datetime(solarDate.year, solarDate.month, solarDate.day)
        diff = numDays.days
        # Count forward if year is the future of referenceYear
        if diff <= 0:
            # Return heavenlyStem, earthlyBranch
...
class Sexagenary(object):
    def __init__(self, year, month, day, solar=True, isleap=False):
        if solar:
            self.solarDate = Solar(year, month, day)
        else:
            lunarDate = Lunar(year, month, day, isleap=isleap)
            self.solarDate = Converter.Lunar2Solar(lunarDate)

        self.year, self.month, self.day = Converter.Solar2Sexagenary(self.solarDate)
        self.yearConverted = SexagenaryBasic.getString(*self.year)
        self.monthConverted = SexagenaryBasic.getString(*self.month)
        self.dayConverted = SexagenaryBasic.getString(*self.day)
